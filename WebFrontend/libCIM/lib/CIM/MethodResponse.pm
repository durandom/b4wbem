...
sub new {
    ...
    $self->{'CIM::MethodResponse::_methodName'} = undef;
    $self->{'CIM::MethodResponse::_returnValue'} = undef;
    $self->{'CIM::MethodResponse::_error'} = undef;
    
    $self->{_identifier} = 'MethodResponse';
    
    $self->{_childAccessors} = {
				RETURNVALUE => 'returnValue',
				PARAMVALUE  => 'paramValue', #TODO: implement!
				ERROR       => 'error',
...
sub toXML {
    ...
    # METHODRESPONSE
    my $mr = $doc->createElement('METHODRESPONSE');
...
CIM::MethodResponse - class encapsulating the CIM METHODRESPONSE
element
...
=head1 SYNOPSIS
 ...
 $mr = CIM::MethodResponse->new( MethodName => $methodName);

 $doc = $mr->toXML();
 $mr2 = CIM::MethodResponse->new( XML => $doc);


=head1 DESCRIPTION

This module represents the CIM METHODRESPONSE message element, which 
defines the response to a CIM extrinsic method invocation.
A MethodResponse contains only a CIM::Error or zero or one 
CIM::ReturnValue and one or more CIM::ParamValues.
