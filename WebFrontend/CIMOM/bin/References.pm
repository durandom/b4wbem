...
sub test1 {
    ...
    return "References with a CIM::Class as query ($objectName)\n    and with default settings"
	if @_;
...
sub test2 {
    ...
    return "References with a non existent CIM::Class as query ($objectName)"
	if @_;
...
sub test3 {
    ...
    return "References with a CIM::Instance as query ($on)\n    and with default settings"
	if @_;
    
    my @asso;
    eval { 
	@asso = $cc->References(ObjectName => $on,
			       );
    };
