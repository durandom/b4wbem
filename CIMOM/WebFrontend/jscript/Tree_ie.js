     ...
     // and sets the required properties.
{
  ...
  {
    document.write("<DIV TOP="+y+" LEFT="+(this.depth*15)+" ID=Item"+this.id+">");

    document.write("<DIV ID=Buttons WIDTH=14 HEIGHT=14>");

    document.write("<DIV ID=Minus VISIBILITY=HIDE WIDTH=14 HEIGHT=14><IMG SRC=/WebFrontend/images/Tree_minus.gif WIDTH=14 HEIGHT=14></DIV>");

    document.write("<DIV ID=Plus WIDTH=14 VISIBILITY=HIDE HEIGHT=14><IMG SRC=/WebFrontend/images/Tree_plus.gif WIDTH=14 HEIGHT=14></DIV>");

    document.write("<DIV ID=Disabled VISIBILITY=INHERIT WIDTH=14 HEIGHT=14><IMG SRC=/WebFrontend/images/Tree_disabled.gif WIDTH=14 HEIGHT=14></DIV>");

    document.write("</DIV>"); // Buttons

    this.layer = this.enclosing.layers['Item'+this.id];

    this.layers = this.layer.layers;

    document.write("<DIV ID=Content LEFT="+(this.layers['Buttons'].x+15)+">"+content+"</DIV>");

    document.write("</DIV>"); // Item
  ...
  re = /\>Ausgang\</;
  if (re.test(content)) this.name = "NODE-SEND";

  re = /\>Eingang\</;
  if (re.test(content)) this.name = "NODE-RECEIVE";
     ...
     // is where to locate the new layer to be placed correctly.
{
  ...
  if (node.tree.browser == 'ns')
  {
    ...
    // Create the layer for all the children.
    document.write("<DIV TOP="+nexty+" VISIBILITY=HIDE ID=Children>");
     ...
     // Handle a mouse down on a plus (expand).
{
  ...
  else if (node.name == 'NODE-SEND')
  {
    location.href='/paul-menu?submenu=SEND';
    return;
  }
  else if (node.name == 'NODE-RECEIVE')
  {
    location.href='/paul-menu?submenu=RECEIVE';
    return;
  }
...
} // function TreeNode_onmousedown_Plus
     ...
     // Handle a mouse down on a minus (collapse).
{
  ...
  return false;
} // function TreeNode_onmousedown_Minus
     ...
     // x and y in param.
{
  ...
  this.browser=(document.layers) ? 'ns' : 'ie';
  this.id = param_id;
  this.children = new Array;
  this.maxChild = 0;
  this.expanded = true;
  ...
  else
  {
      ...
      document.write("<DIV VISIBILITY=HIDE ID="+this.id+"Tree"+left+top+">");

      document.write("<DIV ID=mainLayer>");

      //this.enclosing = document.layers[this.id+"Tree"].layers['mainLayer'];
      this.enclosing = eval('document.all.mainLayer');

      //this.layers = this.enclosing.layers;  layers-object doesn't exist in IE
  ...
  if (this.browser == 'ns')
  {
        ...
        if (re.test(layers1Root.layers[i].id))
        {
          ...
          for (var k = 0 ; k <layers2Len; k++)
          {
            if (re.test(layers2Root.layers[k].id))
            {
              ...
              for (var l = 0 ; l <layers3Len; l++)
              {
                if (layers3Root.layers[l].layers['Content'])
                {
    ...
    else  // MS IE
    {
      ...
      var nodeLayer = eval('document.all.'+this.id+'Tree');
      nodeLayer.style.visibility="inherit";

      /*
       *  Add Background-color-functionality (Author: F.Fox)
       *  Not implemented yet for MS IE
       */
