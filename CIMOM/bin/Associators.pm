...
sub test1 {
    ...
    return "Associators with a CIM::Class as query ($objectName)\n    and with default settings\n    (returns the associated classes)"
	if @_;
...
sub test2 {
    ...
    return "Associators with a non existent CIM::Class as query ($objectName)"
	if @_;
...
sub test3 {
    ...
    return "Associators with a CIM::Instance as query ($on)\n    and with default settings\n    (returns the associated instances)"
	if @_;

    my @asso;
    eval { 
	@asso = $cc->Associators(ObjectName => $on,
				);
    };
